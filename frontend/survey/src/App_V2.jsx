...
console.log("🔥 FORCE REFRESH APP LOADED - COMPLETELY NEW VERSION!");
...
export default function App() {
  ...
  useEffect(() => {
    const loadRepresentatives = async () => {
      try {
        const res = await fetch(`${API_BASE}/users${CACHE_BUST}`, { headers });
        const data = await res.json();
        if (!res.ok) {
          console.error("Failed to load representatives");
          return;
        }
        const reps = data.filter((user) => user.role === "Representative");
  ...
  // Update planned visit - CRITICAL FUNCTION
  const handleUpdatePlannedVisit = async () => {
    console.log("🔥 handleUpdatePlannedVisit called - FORCE REFRESH VERSION!");
    console.log(`📋 Visit Source: ${visitSource}`);
    ...
    if (visitSource === "planned") {
      ...
      const endpoint = `${API_BASE}/dashboard-visits/${selectedDraftId}/draft${CACHE_BUST}`;
      ...
      try {
        ...
        const errorText = "Network error updating planned visit";
        setMessage(errorText);
        setSectionNotice("create", "error", errorText);
        notify("error", errorText);
        return;
        ...
        {visitSource === "planned" ? (
          <>
            ...
            <div className="form-group">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  const match = representatives.find(
                    (rep) => rep.name.toLowerCase().includes(value.toLowerCase())
                  );
                  setVisitForm((prev) => ({
                    ...prev,
                ...
                placeholder="Start typing to search representatives..."
              />
              ...
              <span className="caption">Search and select a representative from the list.</span>
            </div>
            <div className="form-group">
