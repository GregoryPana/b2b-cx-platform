...
class InstallationContractorCreateRequest(BaseModel):
    name: str
...
def _normalize_field_team_members(values: list[str] | None) -> list[str]:
    members: list[str] = []
    seen: set[str] = set()
...
def _read_field_team_members(value) -> list[str]:
    if value is None:
        return []
    if isinstance(value, list):
        return _normalize_field_team_members(value)
...
@router.get("/contractors", dependencies=[Depends(require_roles(*INSTALL_ROLES))])
def list_installation_contractors(
    q: str | None = None,
    limit: int = 50,
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to load installation contractors: {exc}")


@router.post("/contractors", dependencies=[Depends(require_roles(*INSTALL_ROLES))])
def create_installation_contractor(
    payload: InstallationContractorCreateRequest,
    db: Session = Depends(get_db),
):
    ...
    try:
        row = db.execute(
            text(
                """
                INSERT INTO installation_contractors (name, active)
                VALUES (:name, true)
...
def create_installation_survey(
    ...
    if worker_type == "Contractor":
        ...
        if not _contractor_exists(db, contractor_name):
            raise HTTPException(status_code=400, detail="Selected contractor does not exist. Please add it in Contractors first.")
        field_team_members = []
    else:
        contractor_name = None
        if len(field_team_members) == 0:
        ...
        return [
            {
                ...
                "field_team_members": _read_field_team_members(row["field_team_members"]),
...
def email_installation_report(
    ...
    # Render HTML
    report_payload = payload.get("report") if isinstance(payload, dict) else payload
    report_html = render_installation_report_html(report_payload or {}, getattr(current_user, "name", "Unknown User"))
...
def render_installation_report_html(payload: dict, generated_by: str) -> str:
    ...
    contractor_breakdown = payload.get("contractor_breakdown", [])
    ...
    def _install_score_badge(score, score_max=5):
        ...
        try:
            sv = float(score)
            mv = float(score_max or 5)
            grade = get_installation_metric_grade(sv)
            color = grade.get("text", "#475569")
            bg = grade.get("background", "#f8fafc")
            bd = grade.get("border", "#e2e8f0")
            label = f"{int(sv) if sv == int(sv) else sv:.1f} / {int(mv) if mv == int(mv) else mv:.1f}"
    ...
    if survey_detail:
        ...
        for cat, items in grouped.items():
            question_rows_html += (
                f"<tr><td colspan='3' style='background:#f1f5f9;font-weight:700;padding:8px 12px;font-size:12px;color:#334155;text-transform:uppercase;letter-spacing:0.04em'>{cat}</td></tr>"
            )
            for item in items:
                question_rows_html += (
                    f"<tr>"
                    f"<td style='padding:10px 12px;font-size:13px'><span style='font-size:11px;color:#64748b;font-weight:600'>Q{item['question_number']}</span><br>{item['question_text']}</td>"
                    f"<td style='padding:10px 12px;text-align:center'>{_install_score_badge(item['score'])}</td>"
                    f"<td style='padding:10px 12px;text-align:center;font-size:12px;color:#64748b'>{scoring_range}</td>"
                    f"</tr>"
                )
    else:
        for qa in question_averages:
            ...
            question_rows_html += (
                f"<tr>"
                f"<td style='padding:10px 12px;font-size:13px'><span style='font-size:11px;color:#64748b;font-weight:600'>Q{qa['question_number']}</span><br>{qa['question_text']}</td>"
                f"<td style='padding:10px 12px;text-align:center'>{_install_score_badge(avg)}</td>"
                f"<td style='padding:10px 12px;text-align:center;font-size:12px;color:#64748b'>{scoring_range}</td>"
                f"</tr>"
            )
    ...
    for cat in category_averages:
        ...
        category_rows_html += (
            f"<tr>"
            f"<td style='padding:10px 12px;font-size:13px'>{cat['category']}</td>"
            f"<td style='padding:10px 12px;text-align:center'>{_install_score_badge(cat.get('average_score'))}</td>"
            f"</tr>"
        )
...
<body>
  <div style="background:#0b1f3a;color:#fff;padding:28px 32px 24px;border-radius:8px 8px 0 0;margin-bottom:24px">
    <div style="font-size:11px;text-transform:uppercase;letter-spacing:0.08em;color:#94a3b8;margin-bottom:6px">Installation Assessment Report</div>
    <div style="font-size:22px;font-weight:700;margin-bottom:4px">CWS CX — Installation Quality Report</div>
    <div style="font-size:13px;color:#cbd5e1">Generated by: {generated_by} &nbsp;|&nbsp; {datetime.utcnow().strftime('%d %B %Y, %H:%M UTC')}</div>
  </div>

  <div style="border:1px solid #e2e8f0;border-radius:8px;padding:16px 20px;margin-bottom:24px;background:#f8fafc">
    <p style="font-size:13px;font-weight:700;color:#334155;margin:0 0 10px">How to Read This Report</p>
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px">
      <div style="display:flex;align-items:center;gap:10px;padding:10px;background:#fff;border:1px solid #d1fae5;border-radius:6px">
        <span style="display:inline-block;padding:4px 10px;background:#d1fae5;color:#065f46;border-radius:4px;font-size:12px;font-weight:700">4–5</span>
        <div><div style="font-size:12px;font-weight:700;color:#065f46">Excellent</div><div style="font-size:11px;color:#64748b">Very high quality</div></div>
      </div>
      <div style="display:flex;align-items:center;gap:10px;padding:10px;background:#fff;border:1px solid #fcd34d;border-radius:6px">
        <span style="display:inline-block;padding:4px 10px;background:#fef3c7;color:#92400e;border-radius:4px;font-size:12px;font-weight:700">3</span>
        <div><div style="font-size:12px;font-weight:700;color:#92400e">Satisfactory</div><div style="font-size:11px;color:#64748b">Meeting basic expectations</div></div>
      </div>
      <div style="display:flex;align-items:center;gap:10px;padding:10px;background:#fff;border:1px solid #fca5a5;border-radius:6px">
        <span style="display:inline-block;padding:4px 10px;background:#fee2e2;color:#991b1b;border-radius:4px;font-size:12px;font-weight:700">1–2</span>
        <div><div style="font-size:12px;font-weight:700;color:#991b1b">Poor</div><div style="font-size:11px;color:#64748b">Needs immediate attention</div></div>
      </div>
    </div>
  </div>
"""
    ...
    if survey_detail:
        ...
        <table>
          <thead><tr><th>Contractor</th><th>Average Score</th><th>Survey Count</th></tr></thead>
          <tbody>
            {"".join(f"<tr><td>{row['contractor_name']}</td><td class='text-center' style='color:{get_installation_metric_grade(row.get('average_score'))['text']};font-weight:600'>{number(row['average_score'])}</td><td class='text-center'>{row['survey_count']}</td></tr>" for row in contractor_breakdown) or "<tr><td colspan='3'>No contractor survey data for selected scope.</td></tr>"}
          </tbody>
        </table>
