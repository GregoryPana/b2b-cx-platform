...
def render_report_pdf(payload: dict[str, Any], generated_by: str) -> bytes:
    """Generate PDF report matching HTML version with data, charts, and professional styling."""
    ...
    selected_visit = payload.get("selected_visit_info") or {}
    action_points = payload.get("action_points") or []
    yes_no_comparison = payload.get("yes_no_comparison") or []
    category_comparison = payload.get("category_comparison") or []
    business_breakdown = payload.get("business_breakdown") or []
    ...
    report_type = str(filters.get("report_type") or "lifetime")
    survey_questions = payload.get("survey_question_details") or []

    title_style = ParagraphStyle(
        'CustomTitle',
        parent=styles['Heading1'],
        fontSize=24,
        textColor=colors.HexColor("#0056A1"),
        spaceAfter=6,
        leading=28,
    )
    ...
    def color_for_score(score) -> str:
        """Return color for score-based metric grading."""
        try:
            s = float(score)
            if s >= 9:
                return "#22c55e"  # green
    ...
    story.append(Paragraph("Cable &amp; Wireless — CWSCX B2B Report", title_style))
    story.append(Paragraph(
        f"Type: {REPORT_TYPE_LABELS.get(report_type, 'report').replace('-', ' ').title()}",
        styles['Heading3']
    ))
    story.append(Paragraph(f"Generated by: {generated_by}", small_text))
    story.append(Spacer(1, 8))

    header_data = [
        f"Survey: {filters.get('survey_type') or 'B2B'}",
        f"Scope: {_b2b_scope_label(payload).replace('-', ' ').title()}",
        f"Period: {_b2b_date_label(payload).replace('-', ' ')}",
    ]
    story.append(Paragraph(" | ".join(header_data), small_text))
    story.append(Spacer(1, 12))

    summary_rows = [
        ["Metric", "Value"],
        ["Total Visits", str(summary.get("total_visits", 0))],
        ["Total Businesses", str(summary.get("total_businesses", 0))],
        ["Total Responses", str(summary.get("total_responses", 0))],
        ["Average Score", fmt(summary.get("average_score"))],
        ["Status: Approved", str(summary.get("status_counts", {}).get("Approved", 0))],
        ["Status: Pending", str(summary.get("status_counts", {}).get("Pending", 0))],
    ]
    ...
    for key, label in [("nps", "NPS"), ("csat", "CSAT"), ("relationship_score", "Relationship"), ("competitor_exposure", "Competitive Exposure")]:
        values = comparison.get(key) or {}
        metric_rows.append([
            label,
            fmt(values.get("selected"), "%" if key == "csat" else ""),
            fmt(values.get("overall"), "%" if key == "csat" else ""),
        ])

    if any(metric_rows[i][1] != "--" or metric_rows[i][2] != "--" for i in range(1, len(metric_rows))):
        story.append(Paragraph("Key Performance Indicators", heading2_style))
        metric_table = Table(metric_rows, colWidths=[6*cm, 3*cm, 3*cm])
        metric_table.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#0056A1")),
    ...
    if selected_visit:
        story.append(Paragraph("Survey Context", heading2_style))
        context_data = [
        ...
        context_table = Table(context_data, colWidths=[3.5*cm, 11.5*cm])
    ...
    if action_points:
        story.append(Paragraph("Action Points", heading2_style))
        outstanding = [a for a in action_points if a.get("action_status") != "Completed"]
        completed = [a for a in action_points if a.get("action_status") == "Completed"]

        if outstanding:
            story.append(Paragraph("Outstanding", heading3_style))
            ap_rows = [["Business", "Action", "Owner", "Timeline"]]
            for item in outstanding[:20]:
                ap_rows.append([
                    Paragraph(str(item.get("business_name") or "--"), small_text),
                    Paragraph(str(item.get("action_required") or "--"), small_text),
                    Paragraph(str(item.get("action_owner") or "--"), small_text),
                    str(item.get("action_timeframe") or "--"),
                ])
            ap_table = Table(ap_rows, colWidths=[3*cm, 6*cm, 3*cm, 2*cm])
        ...
        if completed:
            story.append(Paragraph("Completed", heading3_style))
            ap_rows = [["Business", "Action", "Owner", "Timeline"]]
    ...
    if yes_no_comparison:
        story.append(Paragraph("Yes/No Question Comparison", heading2_style))
        yn_rows = [["Question", "Selected Yes %", "Overall Yes %"]]
        for item in yes_no_comparison[:10]:
            yn_rows.append([
                Paragraph(f"Q{int(item.get('question_number') or 0)}: {item.get('question_text') or '--'}", small_text),
                fmt(item.get("filtered_yes_percent"), "%"),
                fmt(item.get("overall_yes_percent"), "%") if item.get("overall_yes_percent") is not None else "--",
            ])
        yn_table = Table(yn_rows, colWidths=[8.5*cm, 2.5*cm, 2.5*cm])
        yn_table.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#0056A1")),
            ("TEXTCOLOR", (0, 0), (-1, 0), colors.white),
            ("ALIGN", (0, 0), (-1, -1), "LEFT"),
        ...
        story.append(yn_table)
        story.append(Spacer(1, 12))

    if category_comparison:
        story.append(Paragraph("Category Score Breakdown", heading2_style))
        cat_rows = [["Category", "Selected Avg", "Overall Avg", "Delta"]]
        for item in category_comparison:
            cat_rows.append([
                str(item.get("category") or "--"),
                fmt(item.get("selected_average_score")),
                fmt(item.get("overall_average_score")) if item.get("overall_average_score") is not None else "--",
                fmt(item.get("delta")) if item.get("delta") is not None else "--",
            ])
        cat_table = Table(cat_rows, colWidths=[4*cm, 3*cm, 3*cm, 2*cm])
    ...
    if business_breakdown and report_type != "survey":
        story.append(Paragraph("Business Analytics", heading2_style))
        biz_rows = [["Business", "Visits", "Avg Score", "Latest Date"]]
        for item in business_breakdown[:15]:
            biz_rows.append([
                Paragraph(str(item.get("business_name") or "--"), small_text),
        ...
        biz_table = Table(biz_rows, colWidths=[5*cm, 2*cm, 2.5*cm, 3*cm])
    ...
    if survey_questions and report_type == "survey":
        story.append(PageBreak())
        story.append(Paragraph("Survey Responses", heading2_style))
        sq_rows = [["Q#", "Question", "Answer", "Verbatim"]]
        for item in survey_questions[:30]:
            answer = item.get("answer_text") or (f"{item.get('score')}/{item.get('score_max')}" if item.get("score") is not None else "--")
            sq_rows.append([
                f"Q{item.get('question_number') or '--'}",
                Paragraph(str(item.get("question_text") or "--"), small_text),
                Paragraph(str(answer), small_text),
                Paragraph(str(item.get("verbatim") or "--"), small_text),
            ])
        sq_table = Table(sq_rows, colWidths=[0.8*cm, 5*cm, 2*cm, 5*cm])
