...
def render_report_pdf(payload: dict[str, Any], generated_by: str) -> bytes:
    ...
    summary_text = f"""
    This report covers CWS B2B account manager survey activity from {filters.get('survey_type', 'B2B')} surveys.
    ...
    if survey_questions:
        ...
        sq_data = [["Question", "Answer"]]
        for item in survey_questions[:15]:
            qnum = item.get("question_number", "--")
            qtext = item.get("question_text", "--")[:50]
            answer = item.get("answer_text") or (f"{item.get('score')}/{item.get('score_max')}" if item.get("score") else "--")
            sq_data.append([f"Q{qnum}: {qtext}", str(answer)[:30]])

        sq_table = Table(sq_data, colWidths=[120*mm, 40*mm])
